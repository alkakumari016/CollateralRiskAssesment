...
public class LoanManagementServiceImpl implements LoanManagementService {
	...
	public ResponseEntity<String> approveLoan(Integer applicationId){
		...
		Integer loanProductId=0;
		if(application.getCollateralDetails().equalsIgnoreCase("Real Estate")) {
			collateralId=101;
			loanProductId=1001;
		}
		else if(application.getCollateralDetails().equalsIgnoreCase("Cash")) {
			collateralId=102;
			loanProductId=1002;
		}
		Double emi=(Double)application.getLoanAmount()/(12.0*application.getTenure())+(0.1*application.getLoanAmount());
		cLoan.setLoanId(application.getApplicationId());
		cLoan.setCustomerId(application.getCustomerId());
		cLoan.setLoanPrincipal(application.getLoanAmount());
		cLoan.setTenure(application.getTenure());
		cLoan.setInterest(12.5);
		cLoan.setEmi(emi);
		cLoan.setCollateralId(collateralId);
		cLoan.setLoanProductId(loanProductId);
		System.out.println("form approve loan"+customerLoanRepo.save(cLoan));
	...
	public ResponseEntity<String> rejectLoan(Integer applicationId){
		...
		System.out.println("in reject block");
